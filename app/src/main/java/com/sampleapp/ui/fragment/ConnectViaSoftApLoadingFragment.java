...
public class ConnectViaSoftApLoadingFragment extends LocalConnectionLoadingFragment implements SoftApService.SoftApDeviceConnectionCallback, SoftApService.SoftApDeviceInfoCallback {
    ...
    public void onDeviceInfoReceived(DeviceInfo deviceInfo, List<WiFiNetwork> candidateNetworks) {
        String deviceId = deviceInfo.getDeviceId() == null ? "<unknown>" : deviceInfo.getDeviceId();
    ...
    public void onError(SoftApService.SoftApDeviceConnectionError error) {
        switch (error) {
            case SOFT_AP_NETWORK_NOT_FOUND:
                MobileAppIntelligence.endOnboarding(EndData.createFailure("cant_find_softap_network"));
                Toast.makeText(
                        CirrentApplication.getAppContext(),
                        R.string.cant_find_soft_ap_network,
                        Toast.LENGTH_SHORT
                ).show();
                showFragment(new HomeFragment(), false);
                break;
    ...
    public void onConnectionFailed() {
        ...
        Toast.makeText(CirrentApplication.getAppContext(), msgResId, Toast.LENGTH_SHORT).show();
        MobileAppIntelligence.endOnboarding(EndData.createFailure("cant_connect_to_softap_network"));
        showFragment(new HomeFragment(), false);
    }
